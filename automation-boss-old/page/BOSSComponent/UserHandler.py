...
class UserHandler(object):
   ...
   # def configure_ug_moh(self, params):
        """
        `Description:` Edit 'User group' from Phone system to add new MOH
     ...
     #       self.action_ele.input_text('ug_headerRow_Name', params['Existing_UG'])
     #       self.action_ele.click_element('ug_name_in_grid')
     #       self.action_ele.select_from_dropdown_using_text('UG_HoldMusicId', params['on_hold_music_staff_add.musicDescription'])
     #       self.action_ele.click_element('UG_groupWizard_next')
     #       self.action_ele.click_element('UG_groupWizard_next')
     #       self.action_ele.click_element('UG_groupWizard_next')
     #       self.action_ele.click_element('UG_groupWizard_finish')
    ...
    #  except:
     #       console("Failed to add new moh to user group")
     #       self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)*/
    ...
    def assign_usergroup(self, params):
        ...
        try:
            ...
            self.action_ele.input_text("headerRowExtensionInput", params["userextn"])
    ...
    def unassign_user(self, params):
        try:
            ...
            time.sleep(3)
            self.action_ele.explicit_wait('addressheader')
            self.action_ele.input_text('addressheader', params['phonemac'])
            time.sleep(2)
            self.action_ele.explicit_wait("MatchingServiceAddress")
            self.action_ele.click_element("MatchingServiceAddress")
            self.action_ele.explicit_wait("editphone_tab")
            self.action_ele.click_element("user_profile")
            self.action_ele.select_from_dropdown_using_text("user_profile", "Anonymous (Unassigned)")
            self.action_ele.click_element("user_profile_save")
            self.action_ele.explicit_wait("ok_box")
            self.action_ele.click_element("ok_box")
            time.sleep(3)
            self.action_ele.click_element("addphone_cancel")

        except Exception, e:
            print(e)
            console("Failed to unassign user")
            self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)

    def assign_user_to_phone(self, params):
        try:
            #import pdb;
            #pdb.Pdb(stdout=sys.__stdout__).set_trace()
            self.action_ele.click_element("Phone_system_tab")
            ...
            self.action_ele.explicit_wait("editphone_tab")
            self.action_ele.click_element("user_profile")
            self.action_ele.select_from_dropdown_using_text("user_profile", params['pname'])
            ...
            print(e)
            console("Failed to assign userprofile to phone")
            self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)

    def enable_ringdown_delay(self,params):
        try:
            ...
            time.sleep(2)
            self.action_ele.explicit_wait("headerRowExtensionInput")
            self.action_ele.input_text("headerRowExtensionInput", params["p1extn"])
            self.action_ele.explicit_wait("au_datagrid_usersDataGrid")
            grid_table_row = self._browser.element_finder("au_datagrid_usersDataGrid")
            if grid_table_row:
                print grid_table_row
                ...
                elms[0].click()
                self.action_ele.explicit_wait("ringdown_enabler")
                self.action_ele.click_element("ringdown_enabler")
                self.action_ele.select_from_dropdown_using_text('ringdown_enabler', "Enabled")
                self.action_ele.click_element("ringdown_enabler_confirm")
                #import pdb;pdb.Pdb(stdout=sys.__stdout__).set_trace()
                time.sleep(2)
                self.action_ele.explicit_wait("delay_input")
                self.action_ele.click_element("delay_input")
                self.action_ele.input_text("delay_input_enter", 4)
                self.action_ele.click_element("delay_input_confirm")
                time.sleep(2)
                self.action_ele.explicit_wait("ringdown_number")
                self.action_ele.click_element("ringdown_number")
                self.action_ele.select_from_dropdown_using_text('ringdown_number',params["p2extn"] )
                self.action_ele.click_element("ringdown_confirm")
        except Exception, e:
            print(e)
            console("Failed to enable ringdown delay for phone")
            self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)

    def edit_usergroup(self, params):
        #import pdb;
        #pdb.Pdb(stdout=sys.__stdout__).set_trace()
        try:
            status = False
            self.action_ele.explicit_wait('ug_headerRow_Name')
            self.action_ele.input_text('ug_headerRow_Name', params['user_group_name'])
            if self.query_ele.get_text("ug_name_in_grid") == params['user_group_name']:
                self.action_ele.click_element("ug_name")
                self.action_ele.click_element('UG_groupWizard_next')
                self.action_ele.select_from_dropdown_using_text('UG_DirectedIntercom', params['directedIntercom'])
                self.action_ele.select_from_dropdown_using_text('UG_WhisperPage', params['whisperPage'])
                self.action_ele.select_from_dropdown_using_text('UG_Barge', params['Barge'])
            ...
            print(e)
            console("Failed to edit user group")
            self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)
        return status
