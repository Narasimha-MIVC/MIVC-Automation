*** Settings ***
Documentation    Create user in BOSS to push user to IDMS with new UUID - Add User through assign number partition
...
*** Test Cases ***
Create user in BOSS to push user to IDMS with new UUID - Add User through assign number partition
    [Tags]    DEBUG
    ...
    ${phone_number}=  I find phone number with required status  Available

    Set to Dictionary  ${TestUser}  profile_loc  ${locationName}
    Set to Dictionary  ${TestUser}  request_by  ${request_by}
    Set to Dictionary  ${TestUser}  request_source  Email
    Set to Dictionary  ${TestUser}  ph_number    ${phone_number}

    # Add user from primary partition -> numbers tab
    When I switch to "primary_partition" page
    And I move to "numbers" tab
    ${extn} =  I add user from numbers tab   &{TestUser}
    When I switch to "users" page
    And I verify the User exist in user table created from profiles page  &{TestUser}

    # connect to and fetch the UserUuid from m5db
    When I connect db  &{log_info_m5db}
    ${query}=  set variable  select UserUuid from Person where Username=\'${TestUser["profile_mail"]}\'
    @{data_m5db}=  I query db  ${query}

#    # Get the tenant info from D2 and verify
#    And I connect db  &{log_info_d2db}
#    ${query}=  set variable  SELECT * FROM users WHERE GuiLoginName=\'${TestUser["profile_mail"]}\'
...
#    should be equal  ${data_m5db[0]['AccountGuid']}  ${data_d2db[0]['TenantGUID']}
    ...
    set to dictionary  ${url_info}  condition  ${TestUser["profile_mail"]}?${temp}
    ...
    ...                       AND  I delete user ${TestUser["profile_mail"]} as user "${request_by}"
    ...                       AND  I log off
...
Set Init Env
    ...
    set to dictionary  ${TestUser}  &{ProfilePageUser}
