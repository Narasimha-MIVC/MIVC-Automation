*** Settings ***
Documentation    Publish updates to Account (or attributes) to UC-TDS - Delete APIs - Delete Account
...
*** Test Cases ***
Publish updates to Account (or attributes) to UC-TDS - Delete APIs - Delete Account
    [Tags]    DEBUG
    ...
    ${query}=  set variable  select * from Location where AccountId=${TestContract["account_id"]}
...
#    should be equal  ${data_m5db_accountguid[0]['AccountGuid']}  ${data_d2db[0]['TenantGUID']}

    # close the Contract
    I close orders and contract

    #verify M5DB for Account,Services and location table
    # connect to and fetch the Account Uuid from m5db
    When I connect db  &{log_info_m5db}
    ${query}=  set variable  select AccountGuid from Account where Id=${TestContract["account_id"]}
    @{data_m5db_accountguid_closed}=  I query db  ${query}
    log many  @{data_m5db_accountguid_closed}

    # fetch the service details from m5db
    ${query}=  set variable  select * from service_service where AccountId=${TestContract["account_id"]}
    @{data_m5db_servicedata_closed}=  I query db  ${query}
    log many  @{data_m5db_servicedata_closed}

    # fetch the Location details from m5db
    ${query}=  set variable  select * from Location where AccountId=${TestContract["account_id"]}
    @{data_m5db_locationdata_closed}=  I query db  ${query}
    log many  @{data_m5db_locationdata_closed}

    # Verify the log in UC-TDS/UC-SES/UC-LOC
    And I verify log details of summitcloud db  tds  ${data_m5db_accountguid_closed[0]['AccountGuid']}  &{UC_VM_LOGIN}
    And I verify log details of summitcloud db  ses  ${data_m5db_servicedata_closed[0]['ServiceUuid']}  &{UC_VM_LOGIN}
    And I verify log details of summitcloud db  loc  ${data_m5db_locationdata_closed[0]['LocationUuid']}  &{UC_VM_LOGIN}

    # fetch the TENNANT data from UC
    # Request Format - "http://default:1234@10.32.128.43:18085/v2/tenants/245c68a4-425a-4acf-a6a6-113b480b20ce"
    &{url_info_1}=  copy dictionary  ${UC_TDS_TBL_TENANTS_URL}
    set to dictionary  ${url_info_1}  condition  ${data_m5db_accountguid_closed[0]['AccountGuid']}
    @{data_uc_tennant}=  I fetch data from summit cloud db  &{url_info_1}
    log many  @{data_uc_tennant}
    ...
    &{url_info_2}=  copy dictionary  ${UC_SES_TBL_SERVICES_URL}
    ${condition}=  set variable  services:access:cosmo:telephony/tenants/${data_m5db_accountguid_closed[0]['AccountGuid']}
    set to dictionary  ${url_info_2}  condition  ${condition}
    @{data_uc_SES}=  I fetch data from summit cloud db  &{url_info_2}
    ...
    &{url_info_3}=  copy dictionary    ${UC_LOC_TBL_LOCATIONS_URL}
    set to dictionary  ${url_info_3}  condition  ${data_m5db_locationdata_closed[0]['LocationUuid']}
    @{data_uc_LOC}=  I fetch data from summit cloud db  &{url_info_3}
