*** Settings ***
Documentation    API to expose Event data - Validation of idms delete sync SC with Filter as failed -
...              Success of EventPropagator-eventpropagator to SC Fails
...
*** Test Cases ***
API to expose Event data - Validation of idms delete sync SC with Filter as failed - Success of EventPropagator-eventpropagator to SC Fails
...
#####################################################################
    ...
    When I switch to "users" page
    I delete user ${TestUser["au_username"]} as user "${request_by}"
##############################################################################
   ...
   # connect to and fetch the UserUuid from m5db
    ...
    ${query}=  set variable  select Payload from log_EventPropagation where EntityGuid=\'${data_m5db[0]['UserUuid']}\'
   ...
   # Verify the log in UC-IDMS
    ...
    set to dictionary  ${url_info}  option  failed
    set to dictionary  ${url_info}  StartDate  ${data_m5db_EventPropagation[0]['DateCreated']}
    set to dictionary  ${url_info}  EndDate  ${data_m5db_EventPropagation[1]['DateModified']}
...
#    set to dictionary  ${url_info1}  condition  ${TestUser["au_username"]}?${temp}
#    @{data_uc_IDMS}=  I fetch data from summit cloud db  &{url_info1}
#    log many  @{data_uc_IDMS}
#
#    # Verify the UC data
#    set to dictionary  ${test_data}  UserUuid  ${data_m5db[0]['UserUuid']}
#    And I compare and varify ${test_data} with ${data_uc_IDMS} for table users
