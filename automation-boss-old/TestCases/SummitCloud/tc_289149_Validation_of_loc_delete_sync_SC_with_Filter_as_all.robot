*** Settings ***
Documentation    Validation of loc delete sync SC with Filter as all - Success of EventPropagator-
...              eventpropagator to SC Success
...
*** Test Cases ***
Validation of loc delete sync SC with Filter as all - Success of EventPropagator-eventpropagator to SC Success
    ...
    # Verify the Location related log in UC-LOC once it pushed to DB from Backsync
    And I verify log details of summitcloud db  loc  ${data_m5db_locationdata[0]['LocationUuid']}  &{UC_VM_LOGIN}
...
Set Init Env
    ...
    ${uni_str}=     Generate Random String    8    [LETTERS][NUMBERS]
    #${TestGeoLoc}=    create dictionary

    #set to dictionary  ${TestGeoLoc}  &{geolocation_US}
