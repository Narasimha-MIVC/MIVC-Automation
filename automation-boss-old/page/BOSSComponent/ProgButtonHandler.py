...
class ProgButtonHandler(object):
    ...
    def add_programmable_buttons(self, params):
        ...
        try:
            ...
            if params["user_extn"]:
                ...
                self.action_ele.input_text("headerRowExtensionInput", params['user_extn'])
            ...
            if button_number:
                ...
                # self.action_ele.explicit_wait('User_Ph_Prog_Button_2_IP_Phones')
                self.action_ele.click_element('User_Ph_Prog_Button_' + button_number + '_IP_Phones')
                # self.action_ele.click_element('User_Ph_Prog_Button_2_IP_Phones')
                if button_number != "2":
    ...
    def add_programmable_buttons_sca(self, params):
    ...
    def switch_programmable_buttons(self, email):
        try:
            #import pdb; pdb.Pdb(stdout=sys.__stdout__).set_trace()
            self.common_functionality.switch_page_users()
            ...
            print(e)
            raise AssertionError("Could not properly switch to program buttons page")



    def add_programmable_buttons_bca(self, params):
        try:

            # Navigate to the buttons tab.
            #import pdb; pdb.Pdb(stdout=sys.__stdout__).set_trace()
            self.common_functionality.switch_page_users()
            self.action_ele.clear_input_text('User_Page_HeaderRow_FullName')
            self.action_ele.clear_input_text('email_search')
            self.action_ele.input_text('email_search', params['user_email'])
            self.action_ele.click_element('user_phone_settings')
            self.action_ele.click_element('user_prog_button')
            ...
            if int(button_number) != 2:
                ...
                long_label = 'user_prog_button_' + button_number + '_long_label'
                short_label = 'user_prog_button_' + button_number + '_short_label'
            bca_profile = 'user_prog_button_' + button_number + '_select_bca'
            stack_pos ='user_prog_button_' + button_number + '_stack_pos'
            callerid = 'user_prog_button_' + button_number + '_callerid'
            ...
            if params["SelectBCA"]:
                self.action_ele.explicit_wait(bca_profile)
                bca_names = self.query_ele.get_text_list_from_dropdown(bca_profile)
                for i in range(len(bca_names)):
                    if params["ProfileName"] == bca_names[i][7:]:
                        bca_name = bca_names[i]
                        break
                self.action_ele.select_from_dropdown_using_index(bca_profile, i)
            #import pdb; pdb.Pdb(stdout=sys.__stdout__).set_trace()
            time.sleep(2)
            if params["SelectCallStackPosition"]:
                time.sleep(2)
                self.action_ele.select_from_dropdown_using_text(stack_pos, params["SelectCallStackPosition"])
                time.sleep(2)
            if params["SelectShowCallerIDAlert"]:
                self.action_ele.select_from_dropdown_using_text(callerid, params["SelectShowCallerIDAlert"])
            time.sleep(2)
            ...
            print(e)
            raise AssertionError("Could not properly assign program buttons")



    def remove_programmable_buttons(self, params):
        """
        `Description:` For removing program buttons handler
        ...
        try:
            ...
            print(e)
            raise AssertionError("Could not properly remove program buttons")
