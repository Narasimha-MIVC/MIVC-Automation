...
Update STD2 user info and sync the data with Boss portal
  ...
  #4. Update user
  ${updated_user_fname}=  generate_user_name
  ${updated_user_lname}=  generate_user_name
  ${updated_user_email}=  generate_user_name


   ${updated_ph_num}    ${updated_name}    ${updated_email}=  I update STD2User       ${first_name}       ${updated_user_fname}      ${updated_user_lname}     ${updated_user_email}    ${None}    ${None}

    #5.  do "SYNC NOW"
    Then I switch to "Administration/System/Hybrid/Synchronization" page on std2
#    I switch to std2 page        Administration/System/Hybrid/Synchronization
    ...
    I check user info in Boss after sync with STD2    ${updated_name}   ${updated_ph_num}   ${updated_email}    update_user

    #8. Navigate back to D2 and delete the created user
    set to dictionary  ${login}  TabName  T1
    I switch tab on browser  ${login}
    I switch to "Administration/Users/Users" page on std2
    I delete STD2 User     ${updated_user_fname}
