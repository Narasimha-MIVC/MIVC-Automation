...
#    Set to Dictionary  ${TestUser}    assign_new_number    ${True}
    ...
    # check the closed user status in M5DB
    When I connect db  &{log_info_m5db}
    ${query}=  set variable  select PersonStatus.Name as Status,Person.AccountId,Person.Username,* from Person INNER JOIN PersonStatus ON Person.PersonStatusId=PersonStatus.Id where Username=\'${TestUser["au_username"]}'
    @{data_m5db_user}=  I query db  ${query}
    log many  @{data_m5db_user}
    # check in log_eventpropagation table that user have 2 entry 1 is for created and 1 is for closed user
