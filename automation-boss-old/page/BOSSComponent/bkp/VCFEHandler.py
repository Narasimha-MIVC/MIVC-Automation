...
class VCFEHandler(object):
    ...
    def create_emergency_hunt_group(self, params):
        ...
        `Created by:` Vasuja
         """
        invalid_extn_message = "The extension is not available. Suggested extension"
        try:
            extn = ''
            location = ''
            ...
            for i in range(4):
                try:
                    ...
                    if elements and elements[0].is_displayed():
                        ...
                        wil(self.action_ele.explicit_wait, "pg_editor_panel")
                        wil(self.action_ele.click_element, "pg_editor_panel")
                except:
            ...
            if params["tooltip"]:
                try:
                    self.action_ele.mouse_hover("Emergency_hg_tooltip")
                    self._browser.element_finder("Emergency_hg_tooltip_msg")
                    print("Message 'Active Locations with Emergency Registrations and no Emergency Hunt Group attached, will be listed here.' is displayed")
                    time.sleep(1)
                except Exception:
                    time.sleep(1)
                    raise BossExceptionHandle("message on tooltip is not as required!")
            ...
            try:
                SFD(self.query_ele.get_text_list_from_dropdown, self.action_ele.select_from_dropdown_using_text,
                    'Emergency_huntgroup_location', params['Location'], loop_count=5)
                #self.action_ele.select_from_dropdown_using_text("Emergency_huntgroup_location", params["Location"])
            except:
                SFD(self.query_ele.get_text_list_from_dropdown, self.action_ele.select_from_dropdown_using_index, 'Emergency_huntgroup_location', 1)
                #self.action_ele.select_from_dropdown_using_index("Emergency_huntgroup_location", 0)
                location = self.query_ele.get_text_of_selected_dropdown_option('Emergency_huntgroup_location')
    ...
    def verify_loc_status(self, params):
        ...
        try:
            # Prasanna: Added this
            wil(self.action_ele.click_element, 'phone_system_nav', loop_count=40)
            wil(self.action_ele.click_element, "Emergency_Registration_link")

            # Prasanna: Commented the below for loop
            # for i in range(4):
            ...
            wil(self.action_ele.input_text, 'headerRow_Name', params["loc_name"])
            ...
            #         grid_load = self.action_ele.explicit_wait('Geo_Loc_Grid_Canvas')
            #         console("iteration number: %s" % i)
            #         if not grid_load:
            #             raise Exception("No Location is created with Registered status")
            #         if grid_load:
            #             print("Getting the fields")
            #             columns = grid_load.find_elements_by_tag_name('div')
            #             # check if the element is found
            #             if 0 != len(columns) and columns[3].text == params["exp_state"]:
            #                 print("Location Status: %s" % columns[3].text)
            #                 time.sleep(2)
            #                 status = True
            #                 break
            #     except:
            ...
            grid_load = wil(self._browser.element_finder, 'Geo_Loc_Grid_Canvas', return_value=True)
            if not grid_load:
                raise Exception("No Location is created with Registered status")
            row = grid_load.find_element_by_tag_name('div')
            if not row:
                raise Exception("no row got selected")
            columns = row.find_elements_by_tag_name('div')
            if 0 != len(columns) and columns[2].text == params["exp_state"]:
                print("Location Status: %s" % columns[2].text)
                status = True

            wil(self.action_ele.clear_input_text, 'headerRow_Name')
            #self.action_ele.clear_input_text('headerRow_Status')
        except:
    ...
    def add_geo_location(self, params):
        ...
        try:
            ...
            wil(self.action_ele.click_element, "geo_accountLocationsAddButton")
            ...
            wil(self.action_ele.explicit_wait, "geo_accountLocationsAddButton", loop_count=40)
            # self._browser.waitfor_ajax_complete()
            time.sleep(5)
            status = True
            ...
            print(e.message)
            self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)
            raise AssertionError("Geographic location creation failed:", e.message)
        return status
    ...
    def create_hunt_group(self, params):
        ...
        try:
            ...
            wil(self.action_ele.click_element, "vcfe_add_dropdown", loop_count=40)
            wil(self.action_ele.click_element, "vcfe_hunt_group")
            for i in range(4):
                try:
                    time.sleep(2)
                    elements = self._browser.elements_finder("pg_refresh_btn")
            ...
            if params["grp_member"]:
                ...
                self.action_ele.click_element("vcfe_edit")
                if isinstance(params['grp_member'], list):
                    for extn in params['grp_member']:
                        self.action_ele.input_text('vcfe_HG_Search', extn)
                        self.action_ele.click_element("vcfe_Select_Extn")
                        self.action_ele.clear_input_text('vcfe_HG_Search')
                else:
                    self.action_ele.input_text('vcfe_HG_Search', params["grp_member"])
                ...
                self.action_ele.click_element("vcfe_HG_Back")
            time.sleep(2)
            wil(self.action_ele.explicit_wait, "Emergency_huntgroup_Finishbutton")
            wil(self.action_ele.click_element, "Emergency_huntgroup_Finishbutton")
            time.sleep(4)
            ...
            if errors:
                for error in errors:
                    if "The extension is not available" in error.text:
                        ...
                        wil(self.action_ele.input_text,'vcfe_Huntgrp_Extn', extension)
                        extn = self.query_ele.get_value("vcfe_Huntgrp_Extn")
                        wil(self.action_ele.click_element, 'Emergency_huntgroup_Finishbutton')
                        try:
                            ele = wil(self._browser.elements_finder, 'Emergency_huntgroup_Finishbutton', return_value=True, loop_count=40)
                            if ele:
                                self.action_ele.click_element('Emergency_huntgroup_Finishbutton')
                        except:
                            pass
            ...
            print(e.message)
            self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)
            raise AssertionError("Hunt group creation failed:", e.message)
    ...
    def create_extension_list(self, params):
        ...
        try:
            ...
            if params["extnNumber"]:
                if isinstance(params['extnNumber'], list):
                    for extn in params['extnNumber']:
                        self.action_ele.input_text('vcfe_EL_Search', extn)
                        self.action_ele.click_element("vcfe_Select_Extn")
                        self.action_ele.clear_input_text('vcfe_EL_Search')
                else:
    ...
    def create_pickup_group(self, params):
        ...
        try:
            ...
            self.action_ele.explicit_wait('vcfe_pickup_group_name')
            ...
            if params['pickuploc'] != "None":
                ...
                self.action_ele.select_from_dropdown_using_index('vcfe_pickup_group_location', 1)
    ...
    def add_Auto_Attendant(self, params):
        ...
        `Modified by :` Immani Mahesh,Palla Surya Kumar
        ...
        try:
            count = 0
            params = defaultdict(lambda: '', params)
            wil(self.action_ele.click_element, "Add_VCFE")
            wil(self.action_ele.click_element, "Add_AA")
            for i in range(4):
                try:
                    time.sleep(2)
                    elements = self._browser.elements_finder("pg_refresh_btn")
            ...
            while (count <=20):
                self.action_ele.click_element("pg_refresh_btn")
                extn = wil(self.query_ele.get_value, "Aa_Extension", return_value=True)
                if extn:
                    break
                else:
                    time.sleep(0.5)
                    ...
                    print(count)
            if not extn:
                raise AssertionError("Loading Extension Failed:")
            ...
            if params['Aa_Extension'].lower() == 'blank':
                ...
                time.sleep(1)
                self._browser.waitfor_ajax_complete()
                wil(self.action_ele.click_element, "Aa_finishAA")
                time.sleep(1)
                self._browser.waitfor_ajax_complete()
            ...
            if params['timeout'].lower() == 'blank':
                time.sleep(1)
                # import pdb;
                # pdb.Pdb(stdout=sys.__stdout__).set_trace()
                self.action_ele.clear_input_text("vcfe_multiple_timeout_input_box")
                time.sleep(1)
                self.action_ele.click_element("Aa_finishAA")
                time.sleep(1)
                errors = self._browser.elements_finder('vcfe_multiple_digit_val_errors')
                if errors:
                    for error in errors:
    ...
    def create_custom_schedule(self, params):
        ...
        try:
            status = False
            blank_name_error = "This field is required"
            ...
            params = defaultdict(lambda: '', params)
            self.action_ele.click_element("vcfe_add_dropdown")
            wil(self.action_ele.click_element, "vcfe_custom_schedule")
            time.sleep(3)
            for i in range(4):
                try:
                    time.sleep(2)
            ...
            self.action_ele.click_element("vcfe_pickup_finish")
            time.sleep(2)
            if blank_name_error in self._browser._browser.page_source:
                ...
                self.action_ele.click_element("VCFE_delete_yes")
                return True
            elif name_already_in_use in self.query_ele.get_text("Aa_edit_confirm"):
                self.action_ele.explicit_wait("fnMessageBox_OK")
                time.sleep(1)
                self.action_ele.click_element("fnMessageBox_OK")
                self.action_ele.explicit_wait("vcfe_cancel_button")
                time.sleep(1)
                self.action_ele.click_element("vcfe_cancel_button")
                wil(self.action_ele.explicit_wait, "VCFE_delete_yes")
                time.sleep(1)
                self.action_ele.click_element("VCFE_delete_yes")
                return True
            wil(self.action_ele.explicit_wait, "fnMessageBox_OK")
    ...
    def add_Paging_Group(self, params):
        ...
        try:
            params = defaultdict(lambda: '', params)
            wil(self.action_ele.click_element, "Add_VCFE")
            wil(self.action_ele.click_element, "Add_PG")
            for i in range(4):
                try:
                    time.sleep(2)
                    elements = self._browser.elements_finder("pg_refresh_btn")
                    ...
                    print("Retrying click: %d" % i)
                    pass
            wil(self.action_ele.input_text, "Pg_Name",params["Pg_Name"])
            time.sleep(3)
            count = 0
            while count <=20:
                self.action_ele.click_element("pg_refresh_btn")
            ...
            time.sleep(1)
            wil(self.action_ele.click_element, "fnMessageBox_OK")
            wil(self.action_ele.explicit_wait, "vcfe_extension_textbox")
            time.sleep(1)
            return extn
        except:
...
#regression function
    def verify_pickup_group(self, ext):
        ...
        try:
            ...
            time.sleep(3)
            wil(self.action_ele.input_text, "pg_extension_head",ext)
            wil(self.action_ele.explicit_wait, "pg_extension_number")
    ...
    def verify_edited_pickup_group(self, params):
        ...
        try:
            time.sleep(2)
            params = defaultdict(lambda: '', params)
            wil(self.action_ele.click_element, "vcfe_vcfComponentsGrideditButton")
            time.sleep(2)
            for i in range(4):
                try:
                    time.sleep(2)
    ...
    def edit_paging_group(self, params):
        ...
        try:
            ...
            pg_name_error = "This field is required"
            pg_loc_error = "This field is required"
            status = False
            self.action_ele.click_element("vcfe_vcfComponentsGrideditButton")
            self._browser.waitfor_ajax_complete()
            for i in range(4):
            ...
            if params['Remove_Name'].lower() == "true":
                ...
                time.sleep(1)
                self.action_ele.click_element("pg_edit_finish_button")
                if pg_name_error in self.query_ele.get_text("vcfe_val_error"):
                    self.action_ele.click_element("pgg_cancel_button")
                    self.action_ele.explicit_wait("VCFE_delete_yes")
                    time.sleep(1)
                    self.action_ele.click_element("VCFE_delete_yes")
            ...
            if params['Remove_Location'].lower() == "true":
                ...
                time.sleep(1)
                self.action_ele.click_element("pg_edit_finish_button")
                if pg_loc_error in self.query_ele.get_text("vcfe_val_error"):
                    self.action_ele.click_element("pgg_cancel_button")
                    self.action_ele.explicit_wait("VCFE_delete_yes")
                    time.sleep(1)
                    self.action_ele.click_element("VCFE_delete_yes")
    ...
    def edit_custome_schedule(self, params):
        try:
            ...
            copy_message = "Component was created successfully"
            error = "Failed updating component. Error: ScheduleName - has already been taken"
            blank_error = "Failed creating component. Error: ScheduleName - can't be blank"
            time.sleep(2)
            params = defaultdict(lambda: '', params)
            wil(self.action_ele.explicit_wait,"VCFE_entry_search")
            if params['editName'] != "":
                wil(self.action_ele.input_text, "VCFE_entry_search", params["editName"])
            else:
                pass
            # self._browser._browser.refresh()
            self._browser._browser.execute_script("window.scrollTo(document.body.scrollHeight,0);")
            time.sleep(1)
            check_elements = self._browser.elements_finder("pg_edit_checkbox")
            check_elements[0].click()
            # check_elements[1].click()
            time.sleep(1)
            if params['type']=='copy':
                self.action_ele.click_element("pg_copy_button")
            else:
                self.action_ele.click_element("pg_edit_button")
            for i in range(4):
                try:
            ...
            time.sleep(3)
            self._browser.waitfor_ajax_complete()
            if (message1 in self.query_ele.get_text("Aa_edit_confirm")) or\
                    (copy_message in self.query_ele.get_text("Aa_edit_confirm")):
                self.action_ele.explicit_wait("fnMessageBox_OK")
                time.sleep(1)
                self.action_ele.click_element("fnMessageBox_OK")
                self._browser.waitfor_ajax_complete()
                return True
            elif (blank_error in self.query_ele.get_text("Aa_edit_confirm")) or\
                    (error in self.query_ele.get_text("Aa_edit_confirm")) or \
                    (time_error in self.query_ele.get_text("Aa_edit_confirm")):
                wil(self.action_ele.click_element, "fnMessageBox_OK")
                wil(self.action_ele.click_element,"pgg_cancel_button")
                self.action_ele.explicit_wait("VCFE_delete_yes")
                time.sleep(1)
                self.action_ele.click_element("VCFE_delete_yes")
                time.sleep(2)
    ...
    def delete_vcfe_day_name(self, params):
        try:
            ...
            if params['editName']!="":
                ...
                pass
            self._browser._browser.execute_script("window.scrollTo(document.body.scrollHeight,0);")
            self.action_ele.explicit_wait("pg_edit_checkbox")
            time.sleep(1)
            check_elements = self._browser.elements_finder("pg_edit_checkbox")
            check_elements[1].click()
            time.sleep(1)
    ...
    def edit_extension_list(self,params):
        edit_ext_list_name_blank = "This field is required"
        ...
        try:
            ...
            # check_elements = self._browser.elements_finder("pg_edit_checkbox")
            # check_elements[1].click()
            wil(self.action_ele.click_element, "pg_edit_button")
            time.sleep(1)
            for i in range(4):
                try:
                    self._browser.waitfor_ajax_complete()
            ...
            if params['extnlistname'] == "blank":
                ...
                self.action_ele.click_element("Emergency_huntgroup_Finishbutton")
                time.sleep(2)
                if edit_ext_list_name_blank in self.query_ele.get_text("vcfe_val_error"):
                    self.action_ele.click_element("pgg_cancel_button")
                    self.action_ele.explicit_wait("VCFE_delete_yes")
                    time.sleep(1)
                    self.action_ele.click_element("VCFE_delete_yes")
    ...
    def delete_vcfe_by_name(self, vcfe_name):
        ...
        try:
            status = False
            time.sleep(3)
            wil(self.action_ele.clear_input_text, "pg_extension_head")
            wil(self.action_ele.clear_input_text, "VCFE_entry_search")
            wil(self.action_ele.input_text, "VCFE_entry_search", vcfe_name)
            self.action_ele.explicit_wait("vcfe_checkbox")
            time.sleep(1)
            check_elements = self._browser.elements_finder("vcfe_checkbox")
            check_elements[0].click()
            self.action_ele.explicit_wait("VCFE_delete")
            time.sleep(1)
            self.action_ele.click_element("VCFE_delete")
    ...
    def create_on_hours_schedule(self, params):
        ...
        try:
            message1 = 'Component was created successfully'
            params = defaultdict(lambda: '', params)
            wil(self.action_ele.click_element, "vcfe_add_dropdown")
            wil(self.action_ele.click_element, "ohs_add_OnHours_schedule")
            time.sleep(3)
            for i in range(4):
                try:
                    time.sleep(2)
            ...
            wil(self.action_ele.input_text, "ohs_name", params['scheduleName'])
            if params['timezone']:
                self.action_ele.select_from_dropdown_using_text("vcfe_selScheduleTimezone",params['timezone'])
                time.sleep(2)
            self.action_ele.click_element("Emergency_huntgroup_Finishbutton")
            if params["error_message"]:
                if params["error_message"] in self.query_ele.get_text("Aa_edit_confirm"):
                    wil(self.action_ele.click_element, "fnMessageBox_OK")
                    time.sleep(1)
                    self.action_ele.click_element("vcfe_cancel_button")
                    time.sleep(2)
    ...
    def select_vcfe_component_by_extension(self, params):
        ...
        try:
            ...
            self._browser._browser.refresh()
            for i in range(5):
                wil(self.action_ele.clear_input_text, "vcfe_extension_textbox")
            ...
            wil(self.action_ele.explicit_wait, "vcfe_checkbox")
            time.sleep(1)
            self.action_ele.click_element("vcfe_checkbox")
            self.action_ele.explicit_wait("vcfe_vcfComponentsGrideditButton")
            time.sleep(1)
            return status
        except:
    ...
    def verify_updated_hunt_group_value(self, params):
        ...
        try:
            ...
            if params['HGname']!= '':
                wil(self.action_ele.explicit_wait, "vcfe_huntgrp_Name")
        ...
        try:
            ...
            if params["error_message"]:
                if params["error_message"] in self._browser._browser.page_source:
                    ...
                    return False
            self.action_ele.click_element("fnMessageBox_OK")
            self.action_ele.explicit_wait("VCFE_entry_search")
            time.sleep(1)
            return name, date
        except Exception, e:
            print(e)
    ...
    def edit_holiday_schedule(self, params):
        ...
        try:
            ...
            if (params['type']=='copy') and (params['copy_message'] == self.query_ele.get_text("Aa_edit_confirm")):
                self.action_ele.explicit_wait("fnMessageBox_OK")
                time.sleep(1)
                self.action_ele.click_element("fnMessageBox_OK")
                status = True
    ...
    def edit_on_hours_schedule(self, params):
        ...
        try:
            ...
            self.action_ele.explicit_wait("fnMessageBox_OK")
            if (message1 in self.query_ele.get_text("Aa_edit_confirm")) or \
                    (params['type']=='copy' and params['copy_message'] == self.query_ele.get_text("Aa_edit_confirm")):
                wil(self.action_ele.click_element, "fnMessageBox_OK")
                status = True
    ...
    def verify_emergency_hunt_group(self, params):
        ...
        try:
            ...
            time.sleep(3)
            #self.action_ele.explicit_wait("vcfe_cancel_button")
            wil(self.action_ele.click_element, "vcfe_cancel_button")
    ...
    def assign_ph_number_to_vcfe_component(self, param):
        ...
        try:
            param = defaultdict(lambda: '', param)
            ...
            self.action_ele.explicit_wait("BCA_OK_Button", 40)
            time.sleep(1)
            ...
            try:
                ...
                self.action_ele.click_element("BCA_OK_Button")
                self.action_ele.explicit_wait("PhoneSystem_Edit_Cancel_Button")
                self.action_ele.click_element("PhoneSystem_Edit_Cancel_Button")
                self.action_ele.explicit_wait("BCA_Yes_Button")
                self.action_ele.click_element("BCA_Yes_Button")
                time.sleep(4)
                raise BossExceptionHandle("message on page not as required!")

            if param["auto_Attendant"]:
                self.action_ele.explicit_wait("Ph_System_Ph_Numbers_Destination")
                self.action_ele.input_text("Ph_System_Ph_Numbers_Destination", param["AA_Extension"])
            if param["hunt_Group"]:
                self.action_ele.explicit_wait("Ph_System_Ph_Numbers_Destination")
                self.action_ele.input_text("Ph_System_Ph_Numbers_Destination", param["HGExtn"])

            ph_numbers_grid = wil(self._browser.element_finder, "Grid_Canvas", loop_count=10, return_value=True)
            ...
            for i in range(len(rows)):
                ...
                if (fields[3].text)== "Active":
                    print("Destination type "+fields[8].text+" is verified")
                    break
                else:
    ...
    def validate_vcfe_component_fields(self, params):
        """
        `Description:` This function will validate vcfe component fields.

        `Param:` Dictionary contains field values to be validated.
        ...
        try:
            # import pdb;pdb.Pdb(stdout=sys.__stdout__).set_trace()
            status = False
            boundary_error_message = 'Please enter a value between 1000 and 7000.'
            decimal_error_message = 'Please enter only digits.'
            params = defaultdict(lambda: '', params)
            self.action_ele.click_element("Add_VCFE")
            ...
            if elements[0].is_displayed():
                ...
                self.action_ele.explicit_wait("pg_editor_panel")
                self.action_ele.click_element("pg_editor_panel")
            # added to ensure whether global user location is present

            if (params["verify_global_location"] == "Yes"):
                self.action_ele.explicit_wait('Aa_Location')
                self.action_ele.click_element('Aa_Location')
                loc_list = self._browser.element_finder('Aa_Location')
            ...
            while extn == '':
                ...
                if extn:
                    ...
                    time.sleep(1)
            # extn = self.query_ele.get_value("Aa_Extension")  # is it required to write two times
            if (params["Aa_Location"] == "random"):
                time.sleep(1)
                self.action_ele.select_from_dropdown_using_index("Aa_Location", 1)
            else:
            ...
            if params['defaultValue']:
                default_value = self.query_ele.get_value('vcfe_multiple_timeout_input_box')
                if default_value==params['defaultValue']:
                    status=True
                else:
                    raise Exception("Multiple digit default value verification failed!!!")

            if params["tooltip_info"]:
                tip_info = 'When a "Multiple Digits" destination is chosen for a schedule,' \
                           ' the system will wait for the specified time after each keypress' \
                           ' until the expected number of digits is entered. If a "Multiple ' \
                           'Digits" destination is not specified, the "Timeout" value for each ' \
                           'schedule is used.'
                self.action_ele.mouse_hover("User_Ph_Setting_Sca_Info")
                elm = self._browser.element_finder("User_Ph_Setting_Sca_Info")
                if elm.get_attribute('tooltip') == tip_info:
                    status = True
                else:
                    raise Exception("Multiple digit tool tip info verification failed!!!")

            if params['boundary_value']:
                time.sleep(1)
                # import pdb;
                # pdb.Pdb(stdout=sys.__stdout__).set_trace()
                for x in params['boundary_value']:
                    time.sleep(2)
                    self.action_ele.clear_input_text("vcfe_multiple_timeout_input_box")
                    self.action_ele.input_text("vcfe_multiple_timeout_input_box", x)
                    self.action_ele.click_element("Aa_click_out")
                    err_chk = self._browser.element_finder("vcfe_mdt_errors")
                    if x.isdigit() and 1000 <= int(x) <= 7000:
                        if err_chk.is_displayed():
                            return False
                        else:
                            status=True
                            pass
                    else:
                        if err_chk.text == boundary_error_message:
                            status = True
                            pass
                        else:
                            return False

            if params['decimal_value']:
                time.sleep(1)
                # import pdb;
                # pdb.Pdb(stdout=sys.__stdout__).set_trace()
                for x in params['decimal_value']:
                    time.sleep(2)
                    self.action_ele.clear_input_text("vcfe_multiple_timeout_input_box")
                    self.action_ele.input_text("vcfe_multiple_timeout_input_box", x)
                    self.action_ele.click_element("Aa_click_out")
                    err_chk = self._browser.element_finder("vcfe_decimal_errors")
                    if err_chk.text == decimal_error_message:
                        status = True
                        pass
                    else:
                        return False

            if params['muted_text']:
                expected_text='Enter a value between 1000-7000'
                observed_text = self.query_ele.get_text('aa_label')
                if expected_text==observed_text:
                    status=True
                else:
                    raise Exception("Muted text value verification failed!!!")

            if params['input_field']:
                in_put_field=self._browser.elements_finder('vcfe_multiple_timeout_input_box')
                if in_put_field:
                    self.action_ele.input_text("vcfe_multiple_timeout_input_box", params['input_field'])
                    observed_value = self.query_ele.get_value('vcfe_multiple_timeout_input_box')
                else:
                    raise Exception("Input field for multiple digit timeout is not present!!!")
                if observed_value==params['input_field']:
                    status=True
                else:
                    raise Exception("Verification of input field for multiple digit timeout failed!!!")
            ...
            print(e)
            self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)
            raise AssertionError("Error in validating VCFE components.")
        return status

    def search_vcfe_component_by_extension(self, params):
        """
        `Description:` This function will search vcfe component by extension.

        `Param:` Dictionary contains values to be searched.
        ...
        try:
            ...
            for i in range(5):
                ...
                [self.action_ele.input_text_basic("vcfe_extension_textbox", e) for e in params["vcfe_comp"]]
                time.sleep(1)
                print self.query_ele.get_value_execute_javascript(
                    "document.getElementById('%s').value" % "headerRow_ExtensionData")
                if self.query_ele.get_value_execute_javascript(
                                "document.getElementById('%s').value" % "headerRow_ExtensionData") == params[
                    "vcfe_comp"]:
                    print("Extension is mathched")
                    status = True
                    break
            ...
            print(e)
            self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)
            raise AssertionError("Error in searching vcfe component")
        return status

    def search_vcfe_component_by_name(self, params):
        """
        `Description:` This function will search vcfe component by name.
        ...
        try:
            ...
            for i in range(5):
                #import pdb;pdb.Pdb(stdout=sys.__stdout__).set_trace()
                self.action_ele.clear_input_text("VCFE_entry_search")
                self.action_ele.input_text("VCFE_entry_search", params["Aa_Name"])
                self.action_ele.clear_input_text("vcfe_extension_textbox")
                [self.action_ele.input_text_basic("vcfe_extension_textbox", e) for e in params["Aa_Extension"]]
                time.sleep(1)
                print self.query_ele.get_value_execute_javascript(
                    "document.getElementById('%s').value" % "headerRow_ExtensionData")
                if self.query_ele.get_value_execute_javascript(
                                "document.getElementById('%s').value" % "headerRow_ExtensionData") == params[
                    "Aa_Extension"] and self.query_ele.get_value_execute_javascript(
                                "document.getElementById('%s').value"%"headerRow_ComponentName")==params[
                    "Aa_Name"]:
                    print("Name and Extension mathched")
                    status = True
                    break
                else:
                    print("Name and Extension are not mathched")
        except Exception , e:
            print(e)
            self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)
            raise AssertionError("Error in searching vcfe component")
    ...
    def search_vcfe_component_by_type(self, params):
        """
        `Description:` This function will search vcfe component by type.
        ...
        try:
            ...
            self.action_ele.clear_input_text("vcfe_extension_textbox")
            self.action_ele.clear_input_text("VCFE_entry_search")
            self.action_ele.clear_input_text("VCFE_type_search")
            time.sleep(2)
            for i in range(5):
                self.action_ele.clear_input_text("VCFE_type_search")
                self.action_ele.input_text("VCFE_type_search", params["comp_type"])
                self.action_ele.clear_input_text("vcfe_extension_textbox")
                [self.action_ele.input_text_basic("vcfe_extension_textbox", e) for e in params["Aa_Extension"]]
                time.sleep(1)
                print self.query_ele.get_value_execute_javascript(
                ...
                if self.query_ele.get_value_execute_javascript(
                                "document.getElementById('%s').value" % "headerRow_ExtensionData") == params[
                    "Aa_Extension"] and self.query_ele.get_value_execute_javascript(
                                "document.getElementById('%s').value"%"headerRow_ComponentType")==params[
                    "comp_type"]:
                    print("Name and Type are mathched")
                    status = True
                    break
                else:
                    print("Name and Type are not mathched")
        except Exception , e:
            print(e)
            self.action_ele.takeScreenshot(inspect.currentframe().f_code.co_name)
            raise AssertionError("Error in searching vcfe component")
    ...
    def verify_allignment(self,field_to_be_verify):
        """
        `Description:` This function will verify Language allignment
        ...
        try:
            ...
            self.action_ele.click_element("pg_refresh_btn")
            time.sleep(2)
            if (field_to_be_verify == "Language"):
                if(self._browser.element_finder("vcfe_language_alignment_wrt_custom")):
                    verified = True
                    print("Language Field present After Custom Field")
            if (field_to_be_verify == "Settings"):
                if (self._browser.element_finder("vcfe_settings_allignment_wrt_language") and self._browser.element_finder(
                        "vcfe_settings_allignment_wrt_custom")):
                    time.sleep(2)
                    verified = True
                    print("Setting  Field present After Custom Field and before Lanuage Field")
        except:
            print(field_to_be_verify + " Field is not alligned Properly")
        return verified

    def verify_component_sort_order_on_vcfe_grid(self, params):
        """
        `Description:`This helps to validate the sort operation on extension column in VCFE page
        ...
        try:
            status = False
            vcfe_grid = {'name': 'ComponentName', 'extension': 'ExtensionData', 'type': 'ComponentType'}
            if params['header_name'] == vcfe_grid['extension']:
                self.action_ele.explicit_wait("vcfe_grid_cell")
                time.sleep(1)
                self.action_ele.click_element("vcfe_slick_header_column_extension")
            elif params['header_name'] == vcfe_grid['type']:
                self.action_ele.explicit_wait("vcfe_slick_header_column_type")
                time.sleep(1)
                self.action_ele.click_element("vcfe_slick_header_column_type")
            grid_items = self._browser._browser.execute_script('return this.vcfComponentsGridGrid.getData().getItems()')
            data_list = []
            for items in grid_items:
                data_list.append(items[params['header_name']])
            print len(data_list)
            print data_list

            sorted_list = sorted(data_list)
            print ("Sorted list is: '\n'", sorted_list)
            if sorted_list == data_list:
                status = True
            ...
            if params['header_name'] == vcfe_grid['extension']:
                self.action_ele.explicit_wait("vcfe_slick_header_column_extension")
                time.sleep(1)
                self.action_ele.click_element("vcfe_slick_header_column_extension")
            elif params['header_name'] == vcfe_grid['type']:
                self.action_ele.explicit_wait("vcfe_slick_header_column_type")
            ...
            grid_items = self._browser._browser.execute_script('return this.vcfComponentsGridGrid.getData().getItems()')
            print type(grid_items)
            data_list_rev = []
            for items in grid_items:
                data_list_rev.append(items[params['header_name']])
            print len(data_list_rev)
            print data_list_rev
            reverse_list = sorted(data_list_rev, reverse=True)
            print ("Sorted list in descending order is below: ", reverse_list)
            if reverse_list == data_list_rev:
                status = True
                print("Yes , it is in descending order")
            else:
            ...
            print("Sorted order verification is failed", self.verify_extension_sort_order_on_vcfe_page.__doc__)
        return status

    def add_geo_loc_using_personal_reg(self, user_name, params):
        ...
        try:
            ...
            # Get all the rows
            self.action_ele.explicit_wait("emergency_reg_loc_grid")
            geo_loc_data_grid = self._browser.element_finder("emergency_reg_loc_grid")
            if not geo_loc_data_grid:
                raise Exception("No Element found with the locator")
            if geo_loc_data_grid:
                # Get the fields
                row = geo_loc_data_grid.find_element_by_tag_name('div')
                if not row:
                    raise Exception("No row in the grid")
                columns = row.find_elements_by_tag_name('div')
                #columns = geo_loc_data_grid.find_elements_by_tag_name('div')
                # check if the element is found
                print("Profile Name: %s" % columns[2].text)
                print("Profile Name: %s" % columns[1].text)
                if 0 != len(columns) and columns[1].text == user_name:
                    print("Profile Name: %s" % columns[1].text)
                    columns[0].click()
                    print("The field got selected")
                    self.action_ele.click_element("click_emergency_reg_loc_grid")
                    time.sleep(2)

                    # Adding Personal Location
            self.action_ele.click_element("change_emergency_loc_button")
            time.sleep(2)
            self.action_ele.select_radio_button("personal_loc_radio_button")
            self.action_ele.click_element("geo_emergency_locationWizard_next")
            self.action_ele.select_from_dropdown_using_text("select_profile_add", "Create New Address...")
            self.action_ele.input_text("geo_Address_Address1", params["Address01"])
            self.action_ele.input_text("geo_Address_Address2", params["Address02"])
            self.action_ele.select_from_dropdown_using_text("geo_Address_StateProvinceKey", params["state"])
            self.action_ele.input_text("geo_Address_City", params["city"])
            self.action_ele.input_text("geo_Address_ZipCode", params["Zip"])
            self.action_ele.click_element("geo_emergency_locationWizard_next")
            self.action_ele.click_element("confirm_personal_loc")
            self.action_ele.click_element("geo_emergency_locationDetailsWizard_finish")
            time.sleep(30)
            # verify that location updated as personal
            if (self._browser.element_finder("current_emergency_add_type").get_attribute("innerHTML") == "Personal Registration"):
                time.sleep(1)
                status = True
                print("User Location Update via personal registration")

            else:
                raise Exception("Location update with personal reg failed!!!")
    ...
    def vcfe_default_AA(self, params):
        """
        `Description:`
        ...
        try:
            ...
            self.action_ele.clear_input_text("pg_extension_head")
            self.action_ele.clear_input_text("VCFE_entry_search")
            self.action_ele.input_text("VCFE_entry_search", params)
            time.sleep(1)
            #extn=self.query_ele.get_text('vcfe_default_AA')
            fields=self._browser.elements_finder('vcfe_default_AA')
            extn=fields[2].text
            return extn
        except:
