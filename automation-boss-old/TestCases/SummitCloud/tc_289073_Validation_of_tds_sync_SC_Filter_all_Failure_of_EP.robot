*** Settings ***
Documentation    Validation of tds sync SC with Filter as all - Failure of EventPropagator
...
*** Test Cases ***
Validation of tds sync SC with Filter as all - Failure of EventPropagator
    ...
    should not contain  ${data_m5db_EventPropagation}    ${data_m5db_accountguid[0]['AccountGuid']}

    ${query}=  set variable  select Payload from log_EventPropagation where EntityName in ('Account', 'Profile')
...
#    # fetch the data from UC and it should not be reflected in UC_TDS DB
...
#    should be true  "404" in """${data_uc_tds}"""
    ...
    set to dictionary  ${url_info1}  StartDate  ${data_m5db_accountguid[0]['DateCreated']}
    set to dictionary  ${url_info1}  EndDate  ${data_m5db_accountguid[0]['DateModified']}
