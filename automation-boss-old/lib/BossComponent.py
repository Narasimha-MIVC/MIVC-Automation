...
class BossComponent( aob, vcfe, Bca, ecc, user, service, instance, st_d2, dominator):
	...
	def configure_ug_moh(self, **params):
		...
		if params.keys():
			self.boss_page.user_handler.configure_ug_moh(params)
		else:
			print('Please check that the input parameters have been provided')
    ...
    def unassign_user(self, **params):
        ...
        '''
        if params.keys():
            self.boss_page.user_handler.unassign_user(params)
        else:
            print("Please check that the input parameters have been provided")
    ...
    def assign_user_to_phone(self, **params):
        ...
        '''
        if params.keys():
            self.boss_page.user_handler.assign_user_to_phone(params)
        else:
            print("Please check that the input parameters have been provided")
    ...
    def enable_ringdown_delay(self, **params):
        ...
        '''
        if params.keys():
            self.boss_page.user_handler.enable_ringdown_delay(params)
        else:
            print("Please check that the input parameters have been provided")

    def edit_usergroup(self, **params):
        try:
            status = self.boss_page.user_handler.edit_usergroup(params)
            return status
        except:
            raise AssertionError("Error occured, could not edit user group!!")
