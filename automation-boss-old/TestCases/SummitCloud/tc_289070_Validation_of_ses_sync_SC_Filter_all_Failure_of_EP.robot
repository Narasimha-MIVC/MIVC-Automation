*** Settings ***
Documentation    Validation of ses sync SC with Filter as all - Failure of EventPropagator
...
*** Test Cases ***
Validation of ses sync SC with Filter as all - Failure of EventPropagator
    ...
    ${query}=  set variable  select * from Account where Id=${TestContract["account_id"]}
    ...
    When I connect db  &{log_info_m5db}
    ${query}=  set variable  select EntityGuid from log_EventPropagation where EntityName='Service'
    @{data_m5db_EventPropagation}=  I query db  ${query}
    log many  @{data_m5db_EventPropagation}
    should not contain  ${data_m5db_EventPropagation}    ${data_m5db_servicedata[0]['ServiceUuid']}

    ${query}=  set variable  select Payload from log_EventPropagation where EntityName='Service'
...
#    ${data_uc_ses}=  I fetch data from summit cloud db  &{url_info}
#    log  ${data_uc_ses}
#    should be true  "404" in """${data_uc_ses}"""
    ...
    set to dictionary  ${url_info1}  option  all
    set to dictionary  ${url_info1}  StartDate  ${data_m5db_servicedata[0]['DateCreated']}
    set to dictionary  ${url_info1}  EndDate  ${data_m5db_servicedata[0]['DateModified']}
