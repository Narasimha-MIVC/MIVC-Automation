...
class D2API:
    ...
    def fetch_tenant_specific_users(self, tenantId):
        ...
        user = []
        # LIST_USERS_URL_NEW = r'http://' + self.ip + ':' + self.pno + \
        #                      '/director/users/list?&rows=1000&_filter_tenant_id=' + str(tenantId) + r''
        LIST_USERS_URL_NEW = r'http://' + self.ip + ':' + self.pno + \
                             '/director/users/list?&rows=3000&_filter_tenant_id=' + str(tenantId) + r''
        print(LIST_USERS_URL_NEW)
        result = self.client.get(LIST_USERS_URL_NEW)
        print(result)
        d = json.loads(result.text)
    ...
    def fetch_tenant_specific_user_groups(self, tenantId):
        ...
        ug_List=[]
        LIST_USERS_URL_NEW= r'http://' + self.ip + ':' + self.pno + \
                            '/director/user_groups/list?&rows=3000&_filter_tenant_id=' + str(tenantId) + r''
        result = self.client.get(LIST_USERS_URL_NEW)
        d = json.loads(result.text)
...
if __name__ == "__main__":
    ...
    fo = D2API("10.196.7.180", "admin@team2.com", "Shoreadmin1#")
    t=fo.fetch_tenant_specific_cosv("Maha_UMS")
    print ("cos info:",t)
