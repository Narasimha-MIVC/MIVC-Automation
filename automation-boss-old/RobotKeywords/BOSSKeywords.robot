...
I edit user group
    [Arguments]    &{UserGroupInfo}
    ${result}=  Run Keyword    edit usergroup  &{UserGroupInfo}
    should be true  ${result}
...
I assign User ${userextn} in user group ${userGroupName}
    &{usergroup}=    create dictionary  userextn=${userextn}    userGroupName=${userGroupName}
    Run Keyword    assign usergroup      &{usergroup}

I unassign user from phone ${macAddress}
    &{userinfo}=    create dictionary  phonemac=${macAddress}
    Run Keyword    unassign user      &{userinfo}


I assign ${pname} for ${macAddress} phone
    &{userinfo}=    create dictionary  pname=${pname}    phonemac=${macAddress}
    Run Keyword    assign user to phone      &{userinfo}

I enable ringdown delay for ${p1extn} to extension ${p2extn}
    &{userinfo}=    create dictionary  p1extn=${p1extn}    p2extn=${p2extn}
    Run Keyword    enable ringdown delay      &{userinfo}
...
#I edit on hold music    &{on_hold_music_Info}
#    ${result}=  Run Keyword    configure ug moh  &{on_hold_music_Info}
#    Should be true   ${result}

I assign on hold music ${moh_name}
     ${result}=      run keyword  assign_moh  ${moh_name}
     should be true  ${result}
