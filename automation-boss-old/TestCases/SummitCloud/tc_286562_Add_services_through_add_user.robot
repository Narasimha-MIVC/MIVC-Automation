*** Settings ***
Documentation    Publish Service creation to UC-SES - Add Services through Add User
...
*** Test Cases ***
Publish Service creation to UC-SES - Add Services through Add User
    [Tags]    DEBUG
    ...
    ${ph_num}=  set variable  ${phone_num[3:6]}${phone_num[8:11]}${phone_num[-4:]}
    ...
    log many  @{data_m5db_useruuid}

    # Again fetch service uuid from m5db
    ${query}=  set variable  select ServiceUuid from service_Service where TN=\'${ph_num}\'
    log  ${query}
    @{data_m5db_serviceuuid}=  I query db  ${query}
    log many  @{data_m5db_serviceuuid}
    log  ${data_m5db_serviceuuid[-1]['ServiceUuid']}
    ...
    #  Prasana: Need to check whether this validation is required
