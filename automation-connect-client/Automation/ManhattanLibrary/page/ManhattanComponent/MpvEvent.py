...
class MpvEvent:
    ...
    def save_MPV_meeting_date(self, meeting_date):
        ...
        try:           
            ...
            current_date = self.queryElement._get_attribute("event_MPV_date", "value")            
            ...
            if (chang_date[1] == "day"):
                ...
                if day == '0' :
                    ...
                    new_date = new_date + timedelta(days=day)
    ...
    def verify_meeting_name_and_date(self, params):
        ...
        try:
            ...
            #print("the first one is:", MPV_event_info)
            for i in range(1, len(MPV_event_info)):
                MPV_xpath = xpath_value_orig + "[" + str(i) + "]"
                # print("this is xpath sequences" , MPV_xpath)
                #new_xpath = "li#xpath#" + MPV_xpath
                
                # print("................new xpath is:", new_xpath)
                x = self._browser.get_current_browser()
                i = x.find_element_by_xpath(MPV_xpath)
                #lists = self._browser.elements_finder("new_xpath")
                print(i.text)
                # MPV_name_list = params['name']
                # list of lists. [[event1, event2,event3], [event1, event2]]
                # xpath_for_date= self._browser._map_converter("MPV_event_date")
                # event_date = i.find_element_by_xpath(xpath_for_date["BY_VALUE"])                
                # xpath_for_name= self._browser._map_converter("event_click_modify")
                # event_name = i.find_element_by_xpath(xpath_for_name["BY_VALUE"])
            log.mjLog.LogReporter("ManhattanComponent", "info", "verify_meeting_name_and_date -"
