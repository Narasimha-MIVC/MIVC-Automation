*** Settings ***
Documentation    remove std2 user did and apply it to non user profile did
...
*** Test Cases ***
1.Remove std2 user did and apply it to non user profile did
    [Tags]    DEBUG
    ...
    #9. Remove Did from std2 user
    # Switch to Admin->user
	And I switch to "Administration/Users/Users" page on std2
    ${updated_ph_num}    ${updated_name}    ${updated_email}=  I update STD2User       ${first_name}       ${None}      ${None}     ${None}    ${None}    Remove

    #10. assign User Did to BCA did
    I switch to "Administration/Features/CallControl/BridgedCallAppearances" page on std2
    ${updated_bca_user_name}=  I update STD2 BCA User    ${bca_user_name}       ${None}    ${None}    assign    ${DID_number}

    #11.  do "SYNC NOW"
    Then I switch to "Administration/System/Hybrid/Synchronization" page on std2
    I sync now data between STD2 and Boss
    #12. Delete Non-profile user and STD2 user(STD2 user should be delete after BCA user deletion)
    Then I switch to "Administration/Features/CallControl/BridgedCallAppearances" page on std2
