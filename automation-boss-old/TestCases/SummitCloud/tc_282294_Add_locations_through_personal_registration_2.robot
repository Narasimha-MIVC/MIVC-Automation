*** Settings ***
Documentation    Create Location in BOSS to Push to UC - Add locations through personal registration
...
*** Test Cases ***
Create Location in BOSS to Push to UC - Add locations through personal registration
...
#    Set to Dictionary  ${geolocation_US}    state_code    NY
    ...
    ${query}=  set variable  select LocationUuid from Location where Name=\'${locationName}\';
   ...
   #And I switch to "accountdetails" page
    And I switch to "emergency_locations" page
    And I move to "Users" tab
    ${user_fname}=  Evaluate    '${TestUser["au_firstname"]}'.replace('_','')
    log  ${user_fname}
    ${user_name}=  Set Variable  ${user_fname}${SPACE}${TestUser["au_lastname"]}
    log  ${user_name}
    I add location &{geolocation_02} for ${user_name} through personal registration
    ...
    log  ${user_name}
    When I connect db  &{log_info_m5db}
    ${query}=  set variable  select * from Address where NameOnDoor=\'${user_name}\'
    @{data_m5db_address}=  I query db  ${query}
    log many  @{data_m5db_address}

    # fetch the data from UC IDMS -
    # Request Format - "http://default:1234@10.32.128.43:18085/v2/users/sample.test72@jj.com?type=username"
    &{url_info_1}=  copy dictionary  ${UC_IDMS_TBL_USERS_URL}
    ${temp}=  set variable  type=username
    set to dictionary  ${url_info_1}  condition  ${TestUser["au_username"]}?${temp}
    @{data_uc_new}=  I fetch data from summit cloud db  &{url_info_1}
    log many  @{data_uc_new}

    # Verify the UC data
    set to dictionary  ${test_data}  UserUuid  ${data_m5db[0]['UserUuid']}
    set to dictionary  ${test_data}  address_type  emergency
